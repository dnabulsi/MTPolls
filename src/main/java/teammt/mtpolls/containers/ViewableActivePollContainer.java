...
public class ViewableActivePollContainer extends ImmutableContainer {

    private PollManager pollManager;
    private Map<UUID, Poll> currentlyViewing = new HashMap<>();

    public ViewableActivePollContainer(MLib lib, PollManager pollManager) {
        super(lib);
    ...
    public void onTopClick(InventoryClickEvent event) {
        ...
        if (tag == null || tag.isEmpty())
            ...
            pollManager.vote(player, currentlyViewing.get(player.getUniqueId()), index);
        }
    }
    ...
    private ItemStack getTimeItem(Poll poll) {
        ...
        return new ItemBuilder(Material.CLOCK)
                .mnl("viewable-open-poll-container-time")
                .replaceable("%votes%", poll.getVotes())
                .replaceable("%time%", lib.getTimesAPI()
                        .generateTime(poll.getCreatedAt() + poll.getDuration() - Instant.now().getEpochSecond()))
